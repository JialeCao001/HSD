...
from utils.box_utils import match, log_sum_exp, roc_match, hsd_match
...
class HSDMultiBoxLoss(nn.Module):
    ...
    def forward(self,
        ...
        conf_t0 = torch.LongTensor(num, num_priors)
        ...
        for idx in range(num):
            ...
            if self.num_classes == 2:
                labels = labels > 0
            if use_arm:
                hsd_match(0.5, truths, defaults, self.variance, labels, loc_t, conf_t0, conf_t, idx,
                    arm_loc_data[idx].data, loc_data[idx].data, use_weight=False)
            else:
                roc_match(self.threshold, truths, defaults, self.variance, labels,
                      loc_data[idx].data, loc_t, conf_t0, conf_t, idx)

        loc_t = loc_t.cuda()
        conf_t = conf_t.cuda()
        conf_t0 = conf_t0.cuda()

        # wrap targets
        loc_t = Variable(loc_t, requires_grad=False)
        conf_t = Variable(conf_t, requires_grad=False)
        conf_t0 = Variable(conf_t0, requires_grad=False)
        ...
        if use_arm and filter_object:
            ...
            pos0 = conf_t0 > 0
            pos0[object_score_index.detach()] = 0
        else:
            pos = conf_t > 0
            pos0 = conf_t0 > 0
        num_pos = pos.sum(1, keepdim=True)
        num_pos0 = pos0.sum(1, keepdim=True)
        ...
        if num_pos0.data.sum() > 0:
            pos_idx = pos0.unsqueeze(pos0.dim()).expand_as(loc_data)
            loc_p = loc_data[pos_idx].view(-1, 4)
            loc_t = loc_t[pos_idx].view(-1, 4)
            loss_l = F.smooth_l1_loss(loc_p, loc_t, size_average=False)
            N0 = num_pos0.data.sum()
        else:
